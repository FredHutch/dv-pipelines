...
process TENX_GEX_MAP {
  echo true
  publishDir "$target_path_val/Metadata", mode : 'copy'
  ...
  input:
    ...
    val target_path_val from target_path_val
...
process TENX_COUNT {
  echo true 
  publishDir "$target_path_val/Counts" , mode : 'copy'
  ...
  script:
    ...
    COMMAND="cellranger count --id=$sample.library_id --transcriptome=$gex_reference"
...
process TENX_MATRIX {
  echo true
  publishDir "$target_path_val/Counts/${aggr_out}/out/filtered_feature_bc_matrix" , mode : 'copy'
...
process TENX_VDJ {
  echo true
  publishDir "$target_path_val/VDJ" , mode : 'copy'
